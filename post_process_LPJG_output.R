...
# add a readme to map the Exporatory Cases back to the protocol and original output files.
...
landcover_fractions_from_2025 <- read.table("../notes/landcover_fractions_from_2025.txt",head=TRUE)
...
luforest_categories <- list(
  Quercus = c("Que_ile", "Que_rob", "Que_coc", "Que_pub"),
  Fagus = c("Fag_syl"),
  Pinus = c("Pin_syl", "Pin_hal"),
  Picea = c("Pic_abi", "Pic_sit"),
  Larix = c("Lar_dec"),
...
# add one more for the whole forest:
addthis = data.frame(Luforest= "Forest_sum", MeanWoodDensity = mean(df_wooddens$WoodDensity))

mean_wooddensity_df <- rbind(mean_wooddensity_df,addthis)
  ...
  if(is.null(NATURAL)){
    ...
    if(!("Larix" %in% names(myData@data))){
      ...
      forestcover_long <- forestcover_fractions_from_2025 %>%
        pivot_longer(cols = c(Larix, Picea, Pinus, Fagus, Quercus, otherNL, otherBL), 
                     names_to = "land_use_category", 
                     values_to = "fraction") %>%
...
base_dir <- "/Volumes/My Passport/ForestPaths/sims/sims_backup/sims/" 
processed_folder <- "/Volumes/My Passport/ForestPaths/processed/"

managementslist <- c("base","lightthin", "intensthin","longrot", "shortrot", "rettree", "sfire","ceaseman")#"fertfor")#,"","ccf")
...
for(management in managementslist){
  for(scenario in scenariolist){
    ...
    # Species_richness "NATURAL"
    ############################################################
    # LPJG-file:  cmass_natural, Woody species+ C3G
    # Species richness (number of species)
    var = "Species_richness"
    unitss = "-"
    cat(var)
    ...
    # dbh > 50cm (temperate, mediterranean, atlantic), >40 cm (boreal)
    # [TODO] check with MartJan what his distinction between temperae, mediteranean and boreal is.
    # here I am just using a threshold of 58 lat, threshold simply set by eyeballing Europe, to make the threshold work from where it does not look too threshold-y.
    ...
    # Define the column names you want to set to 0
    cols_to_zero_boreal <- c("0_5", "5_10", "10_15", "15_20", "20_25", "25_30", "30_35", "35_40")
    vet_trees_boreal    <- c(   "40_45"  , "45_50" ,  "50_55",   "55_60"  , "60_65" ,  "65_70" ,  "70_75" ,  "75_80"  ,
                                "80_85" ,  "85_90" ,  "90_95" ,  "95_100" , "100_105", "105_110", "110_115" ,"115_120", "120_125", "125_130" ,"130_135" ,"135_140", "140_145", "145_150", ">150" )
    vet_trees_temperate <- c( "50_55",   "55_60"  , "60_65" ,  "65_70" ,  "70_75" ,  "75_80"  ,
                              "80_85" ,  "85_90" ,  "90_95" ,  "95_100" , "100_105", "105_110", "110_115" ,"115_120", "120_125", "125_130" ,"130_135" ,"135_140", "140_145", "145_150", ">150" )
    
    #preps:
    myData@data$Veteran_trees   <- as.numeric(0.0) #
    # boreal:
    myData@data[which(myData@data$Lat>58.00),]$Veteran_trees   <- rowSums(myData@data[which(myData@data$Lat>58.00),..vet_trees_boreal]) /10000
    
    #temperate
    myData@data[which(myData@data$Lat<=58.00),]$Veteran_trees  <- rowSums(myData@data[which(myData@data$Lat<=58.00),..vet_trees_temperate])  /10000
    ...
    # read in output:
    # mean individual basal area per sizeclass
    myData_BA <- getField(source = source.in,quant="diamstruct_ba_natural")
    dbh_cols <-  4:ncol(myData_BA@data)
    
    # read in output:
    # number of individuals
    myData_N <- getField(source = source.in,quant="diamstruct_natural")
    ...
    myData@data <- myData@data[,c("Lon" ,  "Lat" , "Year","Larix", "Picea", "Pinus", "Fagus", "Quercus", "otherBL", "otherNL","Forest_sum")]
    
    
    # Unit conversion: from C to volume using mean wood density
    species <- c("Larix", "Picea", "Pinus", "Fagus", "Quercus", "otherBL", "otherNL", "Forest_sum")
    for (sp in species) {
    ...
    rm(myData_transp, myData_evap,myData_int,myData_season)
    #############################################################################
    ...
    #first visualisations for output:
    if(plotall){
      plotSpatial(myData,years = 2024,layers= "NPP")
    }
    ...
    #"rename" to correct FP variable name and combine soil evaporation and transpiration
    #myData@data$NBP <- myData@data$NEE
    ...
    #"rename" to correct FP variable name and combine soil evaporation and transpiration
    #myData@data$NEE <- myData@data$NEE-myData@data$Fire-myData@data$Harvest
    ...
    #read in output:
    #myData   <- getField(source = source.in,quant = "mwcont_upper") # using placeholder variable here atm..
    
    
    #"rename" to correct FP variable name and combine soil evaporation and transpiration
    #unitconversion: 
    #unclear whether needed. Check once variable is the correct one.
    myData@data$SM <- myData@data$Forest_sum 
    #first visualisations for output:
    if(plotall){
      plotSpatial(myData,years = 2024,layers= "SM")
    }
    ...
    plotSpatial(myData_natural,years = 2099,layers= "Fire_losses")
    ...
    # Natural mortality
    #[TODO] output currently scaled to a gridcell level, I don't think that is right.
    #########################################################################
    ...
    #read in output:
    myData_harv_ccut     <- getField(source = source.in,quant = "closs_harv_ccut")  # closs_harv_ccut.out  closs_harv_luc.out   closs_harv_thin.out
    ...
    if(plotall){
      ...
      plotSpatial(myData,year=1980,layer="Timber_production")
    ...
    #unitconversion: 
    #unclear whether needed. Check once variable is the correct one.
    myData@data$N_losses <- myData@data$Total #myData@data$leach 
